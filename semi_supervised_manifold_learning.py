...
def diffusion_knn_clustering(knn_adj_matrix, knn_hgraph_dict,
    ...
    hypergraph_cut_objective = lambda vec: eval_hypergraph_cut_fn(hypergraph_objective, vec, s_vector, sparse_h, rank,
                                                                  W, D)
    hypergraph_diff_results = dict(
        {'x': x, 'y': y, 'fx': fx, 'objective': hypergraph_cut_objective, 'type': 'hypergraph'})
...
def PPR_knn_clustering(knn_adj_matrix, knn_hgraph_dict, error_tolerance=0.1,
    ...
    graph_PPR = np.linalg.solve(a=(1 + effective_lambda) * np.eye(n) - D_inv @ knn_adj_matrix,
...
def compare_estimated_labels(method, generate_data, k, num_iterations,
    ...
    if method == 'diffusion':
        ...
        hypergraph_PPR_results, graph_PPR_results = PPR_knn_clustering(knn_adj_matrix,
                                                                       knn_hgraph_dict, error_tolerance=0.1,
        ...
        raise ValueError(f'Method should be one of ["diffusion", "PPR"]. Instead got {method}.')
...
def visualize_labels(method='PPR'):
    ...
    for data_generation, problem_kind in [(generate_spirals, ' Spirals'), (generate_overlapping_rings, ' Rings'),
        ...
        graph_x_out, hypergraph_x_out, data_matrix = compare_estimated_labels(method, data_generation, k,
                                                                              target_iternum, titlestring=None,
